...
CREATE  TABLE IF NOT EXISTS `ride` (
  ...
  `obs` TEXT NULL,
  `estimated` TIME NULL,
  `status` SMALLINT(1) NOT NULL DEFAULT 0  COMMENT '0:created-1:progress-2:finished-3:canceled' ,
  `created` TIMESTAMP NULL,
  `updated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`) ,
  INDEX `fk_ride_from_idx` (`from` ASC) ,
  UNIQUE INDEX `cod_UNIQUE` (`cod` ASC) ,
  CONSTRAINT `fk_ride_from_idx`
    FOREIGN KEY (`from` )
...
CREATE  TABLE IF NOT EXISTS `company` (
	...
	`slogan` VARCHAR(255) NOT NULL DEFAULT 'El Mejor Restaurante de la Ciudad',
...
CREATE  TABLE IF NOT EXISTS `taxi` (
	...
	`uri` VARCHAR(255) NOT NULL ,
	`desc` VARCHAR(255) NOT NULL ,
	`rating` DECIMAL(1,1) NOT NULL DEFAULT 0.0 COMMENT 'rating stars' ,
...
CREATE  TABLE IF NOT EXISTS `taxi_ride` (
  ...
  `estimated` TIME NOT NULL DEFAULT 10 COMMENT 'estimated time' ,
  `obs` TEXT NULL ,
