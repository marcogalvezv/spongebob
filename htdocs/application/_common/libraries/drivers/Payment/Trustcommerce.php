...
class Payment_Trustcommerce_Driver implements Payment_Driver
{
	...
	public function __construct($config)
	{
		...
		if (!extension_loaded('tclink'))
		{
			...
			{
				throw new Exception('payment.no_dlib', $this->tclink_library);
			}
		}
	}
	...
	public function process()
	{
		...
		if (in_array(FALSE, $this->required_fields))
		{
			...
			throw new Exception('payment.required', implode(', ', $fields));
		}
		...
		elseif ($result['status'] == 'decline')
			return 'payment.error payment_Trustcommerce.decline: '.$result[$result['status'].'type'];
		else
			return 'payment.error payment_Trustcommerce: '.$result['status'].'.'.$result['error'];
	}
