...
class Payment_Paypal_Driver implements Payment_Driver {
	...
	// fields within api_connection and api_authorization
	...
	(
		...
		//-- OPTIONAL --//
		'CURRENCYCODE'  => 'USD', // default is USD - only required if other currency needed
		'MAXAMT'        => '',
		...
		'LOCALECODE'    => 'BO', // Defaults to 'US' - list of country codes here: https://www.paypal.com/en_US/ebook/PP_NVPAPI_DeveloperGuide/countrycodes_new.html#1006794
		...
		'HDRIMG'        => 'http://www.pidamosalgo.com/images/logos/pidamosalgo_paypal_logo.png', //header image displayed top left, size: 750px x 90px. defaults to business name in text
	...
	private $api_authrorization_fields = array
	(
		'USER'          => '',
		'PWD'           => '',
		'SIGNATURE'     => '',
	...
	*/
	public function __construct($config)
	{
		...
		(
			...
			&$this->api_authrorization_fields,
		...
		if ($this->test_mode)
		{
			$this->api_authrorization_fields['USER']		= $config['SANDBOX_USER'];
			$this->api_authrorization_fields['PWD']			= $config['SANDBOX_PWD'];
			$this->api_authrorization_fields['SIGNATURE']	= $config['SANDBOX_SIGNATURE'];

			$this->api_connection_fields['ENDPOINT']		= $config['SANDBOX_ENDPOINT'];
			$this->api_connection_fields['PAYPALURL']		= $config['SANDBOX_PAYPALURL'];
		}
		else //LIVE CONFIGURATION SETTINGS
		{
			$this->api_authrorization_fields['USER']		= $config['USER'];
			$this->api_authrorization_fields['PWD']			= $config['PWD'];
			$this->api_authrorization_fields['SIGNATURE']	= $config['SIGNATURE'];

			$this->api_connection_fields['ENDPOINT']		= $config['ENDPOINT'];
			$this->api_connection_fields['PAYPALURL']		= $config['PAYPALURL'];
		}
	...
	*/
	...
	public function process()
	{
		...
		if (in_array(FALSE, $this->required_fields))
		{
			...
			show_error('Paypal.required: '.implode(', ', $fields));
			return false;
		}
		...
		//$paypal_token = $this->session['paypal_token'];
		$paypal_token = $this->session->userdata('paypal_token');
		...
		{
			show_error('Paypal.required: PAYERID');
			return false;
			//throw new Exception('payment.required', 'PAYERID');
		}

		$this->do_express_checkout_payment();
		
		$this->response = array_merge($this->nvp_response_array, $this->get_express_checkout_response);
		...
		{		
			...
			$result = FALSE;
			show_error('PayPal.Error: Response Empty');
		}
		
		//FINISH AND CLEAN TOKEN
	...
	*/
	protected function set_express_checkout()
	{
		...
		if (strtoupper($response_array['ACK']) == 'SUCCESS')
		{
			...
			//$_SESSION['paypal_token'] = urldecode($paypal_token);
			//$this->session['paypal_token'] = urldecode($paypal_token);
			
			$decode_paypal_token = urldecode($paypal_token);
			$this->session->set_userdata('paypal_token',$decode_paypal_token);
			...
			if ($this->set_express_checkout_fields['USERACTION']=='commit')
			{
				...
				$urlredirect = $this->api_connection_fields['PAYPALURL'].$paypal_token.'&useraction=commit';
			...
			else
			{
				...
				$urlredirect = $this->api_connection_fields['PAYPALURL'].$paypal_token;
				header("Location: ".$urlredirect);
				//return TRUE;
			}
		}
		else // Something went terribly wrong...
		{
			...
			//throw new Exception('SetExpressCheckout ERROR', print_r($response_array));
			//Kohana::log('error', print_r('SetExpressCheckout response:'.$response_array));
			//url::redirect($this->api_connection_fields['ERRORURL']);
			
			//show_error('Paypal.SetExpressCheckout.Error: '.implode(', ', $response_array));
			error_log('Paypal.SetExpressCheckout.Error: '.implode(', ', $response_array), 3, "/tmp/paypal-errors.log");
			$urlredirect = $this->api_connection_fields['ERRORURL'];
			header("Location: ".$urlredirect);
		}
	}
	...
	*/
	protected function get_express_checkout()
	{
		...
		/*
		echo "<pre>";
		print_r($this->get_express_checkout_response);
		echo "</pre>";
		*/
		...
		{
			//throw new Exception('GetExpressCheckout ERROR', print_r($this->get_express_checkout_response));
			//Kohana::log('error', Kohana::debug('GetExpressCheckout response:'.$response));
			//url::redirect($this->api_connection_fields['ERRORURL']);
			
			//show_error('Paypal.GetExpressCheckout.Error: '.implode(', ', $this->get_express_checkout_response));
			error_log('Paypal.GetExpressCheckout.Error: '.implode(', ', $this->get_express_checkout_response), 3, "/tmp/paypal-errors.log");
			$urlredirect = $this->api_connection_fields['ERRORURL'];
			header("Location: ".$urlredirect);
		}
	}
	...
	*/
	protected function do_express_checkout_payment()
	{
		...
		if (strtoupper($this->nvp_response_array['ACK']) != 'SUCCESS')
		{
			//throw new Exception('DoExpressCheckoutPayment ERROR', print_r($this->nvp_response_array));
			//Kohana::log('error', Kohana::debug('DoExpressCheckoutPayment response:'.$response));
			//url::redirect($this->api_connection_fields['ERRORURL']);
			
			//show_error('Paypal.DoExpressCheckoutPayment.Error: '.implode(', ', $this->nvp_response_array));
			error_log('Paypal.DoExpressCheckoutPayment.Error: '.implode(', ', $this->nvp_response_array), 3, "/tmp/paypal-errors.log");
			$urlredirect = $this->api_connection_fields['ERRORURL'];
			header("Location: ".$urlredirect);
		}
	}
	...
	*/
	...
	{
		$postdata = http_build_query($this->api_authrorization_fields).'&'.$nvp_str;
		...
		if (curl_errno($ch))
		{
			//throw new Exception('CURL ERROR', print_r(array('curl_error_no' => curl_errno($ch), 'curl_error_msg' => curl_error($ch))));
			...
			$this->session->set_flashdata('curl_error_no',curl_errno($ch));
			$this->session->set_flashdata('curl_error_msg',curl_error($ch));
			
			error_log('CURL ERROR', implode(', ', array('curl_error_no' => curl_errno($ch), 'curl_error_msg' => curl_error($ch))), 3, "/tmp/paypal-errors.log");
			//show_error('CURL ERROR', implode(', 'array('curl_error_no' => curl_errno($ch), 'curl_error_msg' => curl_error($ch))));
			$urlredirect = $this->api_connection_fields['ERRORURL'];
			header("Location: ".$urlredirect);
		}
		else
	...
	*/
    ...
    {
		$subscription = array();
		$subscription = $this->nvp_response_array;
		$datetime = date('Y-m-d h:i:s');
		$subscription['success'] 		= $this->success;
		$subscription['datetime'] 		= $datetime;
		$subscription['response'] 		= $this->get_express_checkout_response;
