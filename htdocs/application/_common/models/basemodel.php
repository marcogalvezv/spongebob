...
abstract class Basemodel extends CI_Model{
	...
	public function save(array $data, $check_fields = false, $ext_where = array()){
        ...
        log_message("debug","*****SAVE".print_r($data,true));
		if(isset($data['id']) && (int)$data['id']){
			...
			if($this->db->where("id", $data['id'])->get($this->_table_name)->num_rows()>0){
                log_message("debug","*****update1".print_r($this->db->last_query(),true));
				return $data['id'];
			}else{
                log_message("debug","*****update2".print_r($this->db->last_query(),true));
				return 0;
			}			
		}else{
			unset($data['id']);
			...
			*/
            ...
            log_message("debug","*****insert".print_r($this->db->last_query(),true));
