...
//		$language = 'es';
		...
		get_layout()->set_layout("layout/basic");
		
/*		get_layout()->add_stylesheets("frontend");
		get_layout()->add_stylesheets('frames');
		get_layout()->add_stylesheets('menu');
...
*/		
	...
	function index($city = "santa-cruz")
	{
		//$city = ucfirst($city);

		//DEFAULT VALUES
		if(empty($city)) $city = "santa-cruz";

		//buscar por el nombre verdadero y categoria
		$country = "Bolivia";

		if(!empty($city)){
			get_layout()->set_title("PidamosAlgo de $city, $country");
		}else{
			get_layout()->set_title("PidamosAlgo Lista de Restaurantes");
		}

		$branches = $this->mbranch->getBranchByUriCityList($city);
