...
class Users extends Membership {
    ...
    {
        get_layout()->enabled(false);
        $this->load->view('radiotaxi/user/viewdriver');
    }

    function viewClient()
    {
        get_layout()->enabled(false);
        $this->load->view('radiotaxi/user/viewclient');
    }
    ...
    function listenerDriver()
    {
        $options['custom_filter'] = "gid = '4'";
        $table = 'v_userprofile';
        $columns = array('selected','uid','name','email','status','signupdate','id');
        $index = 'id';
        get_layout()->enabled(false);
        $this->load->model('datatablemodel','mdatatable');
        $data['result'] = $this->mdatatable->generate($table, $columns, $index, $options);
        //replacing in order to call the respective methods.
        $str = str_replace("v_userprofile_edit","v_userprofile_editdriver",$data['result']);
        echo $str;
    }
    ...
    function ajaxeditdriver()
    {
        ...
        $this->load->view('radiotaxi/user/editdriver', $data);
    }
    ...
    function ajaxeditclient()
    {
        ...
        $this->load->view('radiotaxi/user/editclient', $data);
    }
	...
	function ajaxsave()
	{
		...
		if ($_POST) 
		{
            ...
            if(isset($user['status']))
            {
                ...
                $user['status'] = $user['status']=="Aprovado" ? 1 : 0;
            }
            else
            {
                 $user['status'] = 0;
