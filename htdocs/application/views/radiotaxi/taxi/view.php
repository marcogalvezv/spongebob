...
<div class="mws-panel grid_8">
    <form name="admin_members_form" id="admin_members_form" method="post" class="mws-form">
        <div class="mws-panel-header">
            <span><i class="icon-table"></i>Taxis</span>
        </div>
        <div class="mws-panel-body no-padding">
            <table class="mws-table" id="v_taxi">
                <thead>
                <tr>
        ...
        <footer style="height:40px">
            <div class="mws-button-row">
                <input onclick="v_taxi_edit()" value="Agregar Taxi" type="button" class="btn"/>
...
//TODO: Check if this could be removed
    ...
    $(document).ready(function(){
        oAdminTableUsers = $('#v_taxi').dataTable({
            'bServerSide'    : true,
            'bAutoWidth'     : false,
            // "bJQueryUI": true,
            'sPaginationType': 'full_numbers',
            'sAjaxSource': '<?= base_url()?>radiotaxi/taxi/listener',
            'aoColumns' : [
                { 'sName': 'selected'},
                { 'sName': 'number'},
                { 'sName': 'plate'},
    ...
    function v_taxi_edit(iduser){
        $.ajax({
            type: "post",
            url: "<?=base_url()?>radiotaxi/taxi/ajaxedit",
            dataType: "html",
