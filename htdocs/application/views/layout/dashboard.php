...
$company = $this->Systemmodel->company();
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
<!--ajax Loader-->
<div id="ajaxLoadAni" class="span7" style="display:none;">
	<div id="ajaxBox" class="pull-right">
        ...
        <ul class="nav pull-right left-tablet">
          ...
          <li class="dropdown hover">
            ...
            <ul class="dropdown-menu currency">
				<li><a href="<?= base_url()?>member/dashboard/profile"><i class="icon-pencil"></i> <?=lang('front.layout.header.user.profile');?></a></li>
				<li class="divider"></li>
				<li><a href="<?= base_url()?>user/logout"><i class="icon-lock"></i> <?=lang('front.layout.header.user.close');?></a></li>
            </ul>
          </li>
        </ul>
      </div>
        ...
        <ul class="nav pull-right left-tablet">
          <li class="dropdown hover">
            ...
            <ul class="dropdown-menu currency">
              <li>
				<div id="loginBox">
					<form name="login_form" id="login_form" class="cmxform" method="post" action="<?= base_url()?>user/login">
						<fieldset id="body">
      ...
      <div class="container">
        <nav class="subnav frame75per">
          <ul class="nav-pills categorymenu">
			...
			<? if($user->gid == 6){?>
			<li>
				<a href="<?=base_url()?>farmacorp/dashboard/reverts">Reversiones</a>
            </li>
			<?}?>
            <li><a href="#">Reportes</a>
              <div>
                <ul class="arrow">
                  <li><a href="<?= base_url()?>farmacorp/report/credits">Reporte de Creditos</a></li>
			...
			<?php if($user->gid == 4){?>
            <li>
				<a id="tab_orders" href="javascript:void(0);" onClick="changeTab('<?=base_url()?>delivery/dashboard/orders','orders')">Ordenes</a>
            </li>
            <li><a href="#" onClick="statusMenu(this)">Reportes</a>
              <?/*<div>
                <ul class="arrow">
                  <li><a href="<?= base_url()?>farmacorp/report/credits">Reporte de Creditos</a></li>
                  <li><a href="<?= base_url()?>farmacorp/report/all">Reporte Completo</a></li>
            ...
            <li><a href="#" onClick="statusMenu(this)">Sistema</a>
              <div>
                <ul class="arrow">
			...
			<?php }elseif($user->gid == 3){?>
            <li><a id="tab_requests" href="javascript:void(0);" onClick="changeTab('<?=base_url()?>provider/dashboard/requests','requests')">Solicitudes</a>
              <?/*<div>
                <ul class="arrow">
                  <li><a href="<?=base_url()?>delivery/dashboard/orders">Nuevos</a></li>
                  <li><a href="<?=base_url()?>delivery/dashboard/orders/1">En Proceso</a></li>
                  <li><a href="<?=base_url()?>delivery/dashboard/orders/2">Entregados</a></li>
                </ul>
              </div>*/?>
            </li>
            <li><a id="tab_orders" href="javascript:void(0);" onClick="changeTab('<?=base_url()?>provider/dashboard/orders','orders')">Ordenes</a>
              <?/*<div>
                <ul class="arrow">
                  <li><a href="<?=base_url()?>delivery/dashboard/orders">Nuevos</a></li>
                  <li><a href="<?=base_url()?>delivery/dashboard/orders/1">En Proceso</a></li>
            ...
            <li><a href="javascript:void(0);" onClick="statusMenu(this)">Reportes</a>
              <?/*<div>
                <ul class="arrow">
                  <li><a href="<?= base_url()?>farmacorp/report/credits">Reporte de Creditos</a></li>
                  <li><a href="<?= base_url()?>farmacorp/report/all">Reporte Completo</a></li>
            ...
            <li><a href="#" onClick="statusMenu(this)">Sistema</a>
              <div>
                <ul class="arrow">
                  <li><a class="active" href="<?= base_url()?>user/profile">Perfil</a></li>
                  <li><a href="<?= base_url()?>user/logout">Salir</a></li>
                </ul>
              </div>
            </li>
		...
		<div class="dealerLogo pull-right shadow" style="text-align:right; z-index:1; height: 70px; width: 100px; margin-top:-15px;border: 3px solid #ebebeb; float:right;">
			<img src="<?=base_url()?><?=$company->logo?>" alt="<?=$company->name?>" class="center" width="100%" height="100%" style="background-color:#fff" />
...
<div id="maincontainer">
	<div id="tabcontent" class="container shadow BgWhite pa1">
		<?php echo $content; ?>
	</div>
</div>
...
$(document).ready(function() {
	...
	/*ICON DETAILS IN DATATABLE provrequeststable*/
    $('#provrequeststable tbody td img.icondetails').live( 'click', function () {
        var nTr = $(this).parents('tr')[0];
        if ( oAdminTableProviderRequests.fnIsOpen(nTr) )
        {
            /* This row is already open - close it */
            this.src = "<?=base_url();?>images/elements/icons/details_open.png";
            oAdminTableProviderRequests.fnClose( nTr );
        }
        else
        {
            /* Open this row */
            this.src = "<?=base_url();?>images/elements/icons/details_close.png";
            oAdminTableProviderRequests.fnOpen( nTr, fnFormatDetails(nTr), 'details' );
        }
    } );

	/*ICON DETAILS IN DATATABLE provrequeststable*/
    $('#providerorderstable tbody td img.icondetails').live( 'click', function () {
        var nTr = $(this).parents('tr')[0];
        if ( oFrontTableProviderOrders.fnIsOpen(nTr) )
        {
            /* This row is already open - close it */
            this.src = "<?=base_url();?>images/elements/icons/details_open.png";
            oFrontTableProviderOrders.fnClose( nTr );
        }
        else
        {
            /* Open this row */
            this.src = "<?=base_url();?>images/elements/icons/details_close.png";
            oFrontTableProviderOrders.fnOpen( nTr, fnFormatDetails(nTr), 'details' );
        }
    } );

	/*ICON DETAILS IN DATATABLE provrequeststable*/
    $('#orderdeliverytable tbody td img.icondetails').live( 'click', function () {
        var nTr = $(this).parents('tr')[0];
        if ( oAdminTableDeliveryOrder.fnIsOpen(nTr) )
        {
            /* This row is already open - close it */
            this.src = "<?=base_url();?>images/elements/icons/details_open.png";
            oAdminTableDeliveryOrder.fnClose( nTr );
        }
        else
        {
            /* Open this row */
            this.src = "<?=base_url();?>images/elements/icons/details_close.png";
            oAdminTableDeliveryOrder.fnOpen( nTr, fnFormatDetails(nTr), 'details' );
        }
    } );
...
*/
	...
	function setValidNumber(object)
	{
		numero = object.value;
		if ((numero == '') || (numero == '0') ||(parseInt(numero) < 0))
			object.value = '1';
	}

/*
***************************************
* tab or menu functions
***************************************
*/
function statusMenu(object) {
/*	$.cookie("pidamosstatusmenu", status, { expires: 1, path: '/' });*/

	$('ul.categorymenu li a').removeClass('active');
...
function settabsession(tab){
	$.ajax({
			type: "get",
			url: "<?=base_url()?>provider/dashboard/ajaxtabs/"+tab ,
			async : false,
...
$(function() {
	...
	<?if ($tabcurrent == "requests"){?>
		changeTab('<?=base_url()?>provider/dashboard/requests','requests');
	<?}elseif ($tabcurrent == "orders"){?>
		<?php if($user->gid == 3){?>
			changeTab('<?=base_url()?>provider/dashboard/orders','orders')
		<?php }elseif($user->gid == 4){?>
			changeTab('<?=base_url()?>delivery/dashboard/orders','orders')
		<?}?>	
	<?}else{?>
		<?php if($user->gid == 3){?>
			changeTab('<?=base_url()?>provider/dashboard/requests','requests');
