...
<div class="mws-panel grid_8">
    <form name="admin_members_form" id="admin_members_form" method="post" class="mws-form">
        <div class="mws-panel-header">
            <span><i class="icon-table"></i>Empresas</span>
        </div>
        <div class="mws-panel-body no-padding">
            <table class="mws-table" id="v_companytable">
                <thead>
                <tr>
                    <th "width: 5%"><?= lang('company.tab.select');?></th>
                    <th "width: 5%"><?= lang('company.tab.uid');?></th>
                    <th "width: 15%"><?= lang('company.tab.name');?></th>
                    <th "width: 10%"><?= lang('company.tab.rating');?></th>
                    <th "width: 7%"><?= lang('company.tab.email');?></th>
                    <th "width: 7%"><?= lang('company.tab.numtaxis');?></th>
                    <th "width: 15%" ><?= lang('company.tab.lat');?></th>
                    <th "width: 18%" ><?= lang('company.tab.lng');?></th>
        ...
        <footer style="height:40px">
            <div class="mws-button-row">
                <input onclick="v_userprofile_editcorporativo()" value="<?= lang('users.tab.add');?>" type="button" class="btn"/>
...
//TODO: Check if this could be removed
    ...
    $(document).ready(function(){
        oAdminTableUsers = $('#v_companytable').dataTable({
            'bServerSide'    : true,
            'bAutoWidth'     : false,
            // "bJQueryUI": true,
            'sPaginationType': 'full_numbers',
            'sAjaxSource': '<?= base_url()?>admin/users/listenerCompany',
            'aoColumns' : [
                { 'sName': 'selected'},
                { 'sName': 'uid'},
                { 'sName': 'name'},
